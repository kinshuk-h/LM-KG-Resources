...
class AproximatedEditDistance(GraphEditDistance):
    ...
    def cost_matrix(self, g1, g2):
        ...
        # Insertion
        cost_matrix[len(g1):, 0:len(g2)] = np.inf
        np.fill_diagonal(cost_matrix[len(g1):, 0:len(g2)], self.node_insertion(g1)+self.edge_insertion(g1.adj.values()))

        # Deletion
        cost_matrix[0:len(g1), len(g2):] = np.inf
        np.fill_diagonal(cost_matrix[0:len(g1), len(g2):], self.node_deletion(g2)+self.edge_deletion(g2.adj.values()))
