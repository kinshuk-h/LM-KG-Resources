...
class HausdorffEditDistance(GraphEditDistance):
    ...
    def ged(self, g1, g2):
        ...
        # Deletion
        d1 = self.node_deletion(g1) + self.edge_deletion(g1.adj.values())/2

        # Insertion
        d2 = self.node_insertion(g2) + self.edge_insertion(g2.adj.values())/2

        # Substitution
        cs = self.node_substitution(g1, g2)

        # edges can be text-named
        g1edges = list(g1.adj.keys())
        g2edges = list(g2.adj.keys())

        for i in range(len(d1)):
            for j in range(len(d2)):
                g1v = g1.adj[g1edges[i]].values()
                g2v = g2.adj[g2edges[j]].values()
